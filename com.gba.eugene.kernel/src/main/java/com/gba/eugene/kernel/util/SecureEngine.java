...
public class SecureEngine {
    private static final Logger log = LoggerFactory.getLogger(SecureEngine.class);
    /** Test String					*/
    ...
    private SecureEngine(String className){
        ...
        if (!testC.equals(TEST))
            throw new IllegalStateException(realClass
                    + ": " + TEST
                    + "->" + testE + "->" + testC);
        log.trace (realClass + " initialized - " + implementation);
    }
    ...
    public static void init (Properties ctx)
    {
        if (s_engine == null)
        {
            String className = ctx.getProperty(SecureInterface.SYSTEM_SECURE);
